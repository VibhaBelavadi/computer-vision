...
int detect(Mat frame,
	...
	*/
	...
	for (int i = 0; i < nfaces; i++) {
		...
		Rect lower_face = Rect(x1, y1, face.width, face.height / 2.33);
		drawEllipse(frame, lower_face, 100, 0, 255);
		Mat lower_faceROI = frame_gray(lower_face);
		*/
		float x1 = face.x + (face.width / 3.4); 
		float y1 = face.y + 2 * face.height / 3;
		Rect lower_face = Rect(x1, y1, 0.55*face.width, (face.height / 3) + 
			(face.height / 13)); 
		drawEllipse(frame, lower_face, 100, 0, 255);
		Mat lower_faceROI = frame_gray(lower_face);
